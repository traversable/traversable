...
deriveValidator.fold = (
  ...
  fn.flow(Sort.derive({ compare }), Schema.fromSchema, fn.para(Schema.functor)(RAlgebra.validator), (xf) =>
    xf.go([], 0, true),
  )
 ...
 */
function deriveValidator(schema: Schema | Schema.Weak | Ltd, options?: Options): string
function deriveValidator(
  schema: Schema | Schema.Weak | Ltd,
  {
    functionName,
