...
import { Kind as HKT, Functor, symbol, URI, newtype } from "@traversable/registry"
...
const isObject = (u: unknown): u is { [x: string]: unknown } => u !== null && typeof u === "object" && !Array_isArray(u)
...
export declare namespace JsonSchema {
  ...
  interface allOf { allOf: readonly JsonSchema[] }
  namespace allOf { interface $<T> { allOf: readonly T[] } }
  interface anyOf { anyOf: readonly JsonSchema[] }
  namespace anyOf { interface $<T> { anyOf: readonly T[] } }
  interface oneOf { oneOf: readonly JsonSchema[] }
  namespace oneOf { interface $<T> { oneOf: readonly T[] } }
  interface array { type: "array", items: JsonSchema }
  namespace array { interface $<T> { type: "array", items: T } }
  interface Schema_object { type: "object", properties: { [x: string]: JsonSchema } }
  namespace Schema_object { interface $<T> { type: "object", properties: { [x: string]: T } } }
  /// 
  type Scalar =
    | JsonSchema.null
    | JsonSchema.boolean
    ...
    | JsonSchema.string
    ;
  interface Kind<T = unknown> extends HKT<T> { ["~1"]: JsonSchema.$<this["~0"]> }
  type $<T> =
    | JsonSchema.null
    | JsonSchema.boolean
    | JsonSchema.integer
...
export namespace JsonSchema {
  ...
  export function map<S, T>(f: (s: S) => T): (s: HKT.apply<JsonSchema.Kind, S>) => HKT.apply<JsonSchema.Kind, T> {
    return (x: HKT.apply<JsonSchema.Kind, S>) => {
      switch (true) {
        case JsonSchema.is.null(x): return x
        case JsonSchema.is.boolean(x): return x
        case JsonSchema.is.integer(x): return x
        case JsonSchema.is.number(x): return x
        case JsonSchema.is.string(x): return x
        ...
        case JsonSchema.is.object(x): return { 
          ...x, 
          properties: Object_fromEntries(
            Object_entries(x.properties).map(([k, v]) => [k, f(v)])
          )
        }
  ...
  export namespace Children {
    ...
    export const object: <const T extends { properties: unknown }>(n: T) => T["properties"] = (n) => n.properties
    export const array: <const T extends { items: unknown }>(n: T) => T["items"] = (n) => n.items
  }

  /// TODO: make this typeguard not a liar
  export function is(u: unknown): u is JsonSchema { return true }
  ...
  export namespace is {
    ...
    export const object = <T>(u: unknown): u is JsonSchema.object.$<T> => isObject(u) && u.type === "object" && !!u.properties
  ...
  export namespace make {
    ...
    export const record = <T>(additionalProperties: T): Ext.record.$<T> => ({ type: "record", additionalProperties })
    export const object = <T>(properties: { [x: string]: T }): Ext.object.$<T> => ({ type: "object", properties })
  }
  const make_null = (): JsonSchema.null => ({ type: "null" })
  void (make.null = make_null);
  export declare namespace make { export { make_null as null }}

  export namespace Algebra {
    export const count
      : Functor.Algebra<JsonSchema.Kind, number>
      = (n) => {
        switch (true) {
          case JsonSchema.is.null(n): return 1
          case JsonSchema.is.boolean(n): return 1
          case JsonSchema.is.integer(n): return 1
          case JsonSchema.is.number(n): return 1
          case JsonSchema.is.string(n): return 1
          case JsonSchema.is.array(n): return 1 + n.items
          case JsonSchema.is.allOf(n): return 1 + n.allOf.reduce((out, x) => out + x, 0)
          case JsonSchema.is.anyOf(n): return 1 + n.anyOf.reduce((out, x) => out + x, 0)
          case JsonSchema.is.oneOf(n): return 1 + n.oneOf.reduce((out, x) => out + x, 0)
          case JsonSchema.is.object(n): return 1 + Object_values(n.properties).reduce((out, x) => out + x, 0)
    ...
    export const depth
      ...
      = (n) => {
        switch (true) {
          ...
          case JsonSchema.is.string(n): return 1
          case JsonSchema.is.array(n): return 1 + n.items
          case JsonSchema.is.allOf(n): return 1 + Math_max(...n.allOf)
          case JsonSchema.is.anyOf(n): return 1 + Math_max(...n.anyOf)
          case JsonSchema.is.oneOf(n): return 1 + Math_max(...n.oneOf)
          case JsonSchema.is.object(n): return 1 + Math_max(...Object_values(n.properties))
          default: return fn.exhaustive(n)
        }
      }

    export const paths
      : Functor.Algebra<JsonSchema.Kind, readonly (readonly (keyof any)[])[]>
      = (n) => {
        switch (true) {
          case JsonSchema.is.null(n): return [[n.type]]
          case JsonSchema.is.boolean(n): return [[n.type]]
          case JsonSchema.is.integer(n): return [[n.type]]
          case JsonSchema.is.number(n): return [[n.type]]
          case JsonSchema.is.string(n): return [[n.type]]
          case JsonSchema.is.allOf(n): return n.allOf.map((xss) => xss.flatMap((xs, ix) => [symbol.intersection, ix as keyof any].concat(xs)))
          case JsonSchema.is.anyOf(n): return n.anyOf.map((xss) => xss.flatMap((xs, ix) => [symbol.union, ix as keyof any].concat(xs)))
          case JsonSchema.is.oneOf(n): return n.oneOf.map((xss) => xss.flatMap((xs, ix) => [symbol.disjoint, ix as keyof any].concat(xs)))
          case JsonSchema.is.array(n): return n.items.map((xs) => [symbol.array as keyof any].concat(xs))
          case JsonSchema.is.object(n): 
            return Object_entries(n.properties).flatMap(([k, xss]) => xss.map((xs) => [k as keyof any].concat(xs)))
          default: return fn.exhaustive(n)
        }
      }
  }
...
export type Ext =
  ...
  | Ext.record
  ;
export declare namespace Ext { export { Ext_object as object } }
export declare namespace Ext {
  interface allOf { allOf: readonly Ext[] }
  namespace allOf { interface $<T> { allOf: readonly T[] } }
  interface anyOf { anyOf: readonly Ext[] }
  namespace anyOf { interface $<T> { anyOf: readonly T[] } }
  interface oneOf { oneOf: readonly Ext[] }
  namespace oneOf { interface $<T> { oneOf: readonly T[] } }
  interface array { type: "array", items: Ext }
  namespace array { interface $<T> { type: "array", items: T } }
  interface tuple { type: "tuple", items: readonly Ext[] }
  namespace tuple { interface $<T> { type: "tuple", items: readonly T[] } }
  interface record { type: "record", additionalProperties: Ext }
  namespace record { interface $<T> { type: "record", additionalProperties: T } }
  interface Ext_object { type: "object", properties: { [x: string]: Ext } }
  namespace Ext_object { interface $<T> { type: "object", properties: { [x: string]: T } } }
  /// 
  interface Kind<T = unknown> extends HKT<T> { ["~1"]: Ext.$<this["~0"]> }
  type $<T> =
    | JsonSchema.null
    | JsonSchema.boolean
    | JsonSchema.integer
...
export namespace Ext {
  ...
  export const functor: Functor<Ext.Kind> = { map }
  ...
  export function fold<T>(algebra: Functor.Algebra<Ext.Kind, T>) /// impl.
    { return fn.cata(functor)(algebra) }
  ...
  export namespace Algebra {
    export const count
      : Functor.Algebra<Ext.Kind, number>
      = (n) => {
        switch (true) {
          case JsonSchema.is.null(n): return 1
          case JsonSchema.is.boolean(n): return 1
    ...
    export const paths
      : Functor.Algebra<Ext.Kind, (keyof any)[][]>
      = (n) => {
        switch (true) {
          case JsonSchema.is.null(n): return [[n.type]]
          case JsonSchema.is.boolean(n): return [[n.type]]
          ...
          case JsonSchema.is.number(n): return [[n.type]]
          case JsonSchema.is.string(n): return [[n.type]]
          case Ext.is.allOf(n): return n.allOf.map((xss) => xss.flatMap((xs, ix) => [ix as keyof any].concat(xs)))
          case Ext.is.anyOf(n): return n.anyOf.map((xss) => xss.flatMap((xs, ix) => [ix as keyof any].concat(xs)))
          case Ext.is.oneOf(n): return n.oneOf.map((xss) => xss.flatMap((xs, ix) => [ix as keyof any].concat(xs)))
  ...
  export namespace tmp {
    export type ArrayLike = { type: "array", items: tmp | tmp[] }
    export type ObjectLike = 
      | { type: "object", properties: globalThis.Record<string, tmp> }
      | { type: "object", additionalProperties: tmp }
    export const isArrayLike = (u: unknown): u is ArrayLike => isObject(u) && u.type === "array"
    export const isObjectLike = (u: unknown): u is tmp.ObjectLike => isObject(u) && u.type === "object"
  }

  export namespace Coalgebra {
    export const fromSchema
      : Functor.Coalgebra<Ext.Kind, tmp>
      = (expr) => {
        switch (true) {
          case JsonSchema.is.null(expr): return JsonSchema.make.null()
          case JsonSchema.is.boolean(expr): return JsonSchema.make.boolean()
          case JsonSchema.is.integer(expr): return JsonSchema.make.integer()
          case JsonSchema.is.number(expr): return JsonSchema.make.number()
          case JsonSchema.is.string(expr): return JsonSchema.make.string()
          case JsonSchema.is.allOf(expr): return Ext.make.allOf(expr.allOf)
          case JsonSchema.is.anyOf(expr): return Ext.make.anyOf(expr.anyOf)
          case JsonSchema.is.oneOf(expr): return Ext.make.oneOf(expr.oneOf)
          case tmp.isObjectLike(expr): 
            return "properties" in expr 
              ? Ext.make.object(expr.properties) 
              : Ext.make.record(expr.additionalProperties)
          case tmp.isArrayLike(expr): 
            return Array_isArray(expr.items) 
              ? Ext.make.tuple(expr.items) 
              : Ext.make.array(expr.items)
  ...
  // TODO: fix this type! the problem is that `FirstPass` has not been fully instantiated.
  export const fromSchema 
    : <const T>(term: T) => fromSchema<T>
    = Ext.unfold(Ext.Coalgebra.fromSchema) as never

  export type fromSchema<T>
    = T extends JsonSchema.Scalar ? T
    : T extends { allOf: infer S } ? { allOf: fromSchema<S> }
    : T extends { anyOf: infer S } ? { anyOf: fromSchema<S> }
    : T extends { oneOf: infer S } ? { oneOf: fromSchema<S> }
    : T extends { properties: infer S } ? { type: "object", properties: fromSchema<S> }
    : T extends { additionalProperties: infer S } ? { type: "record", additionalProperties: fromSchema<S> }
    : T extends { items: infer S extends readonly unknown[] } ? { type: "tuple", items: { [K in keyof S]: fromSchema<S[K]> } }
    : T extends { items: infer S } ? { type: "array", items: fromSchema<S> }
    : never
    ;
}
