...
const isObject 
  ...
  = (u): u is never => u !== null && typeof u === "object" && !Array_isArray(u)
...
export declare namespace Ltd {
  ...
  interface Ltd_integer { type: "integer" }
  interface allOf { allOf: readonly Ltd[] }
  namespace allOf { interface F<T> { allOf: readonly T[] } }
  interface anyOf { anyOf: readonly Ltd[] }
  namespace anyOf { interface F<T> { anyOf: readonly T[] } }
  interface oneOf { oneOf: readonly Ltd[] }
  namespace oneOf { interface F<T> { oneOf: readonly T[] } }
  interface array { type: "array", items: Ltd }
  namespace array { interface F<T> { type: "array", items: T } }
  interface Schema_object { type: "object", properties: { [x: string]: Ltd } }
  namespace Schema_object { interface F<T> { type: "object", properties: { [x: string]: T } } }
  ///
  type Scalar = Ltd.null | Ltd.boolean | Ltd.integer | Ltd.number | Ltd.string
  interface lambda<T = unknown> extends HKT<T> { [-1]: Ltd.F<this[0]> }
  type F<T> =
    | Ltd.null
    | Ltd.boolean
    | Ltd.integer
...
export namespace Ltd {
  ...
  export function map<S, T>(f: (s: S) => T): (s: HKT.apply<Ltd.lambda, S>) => HKT.apply<Ltd.lambda, T> {
    return (x: HKT.apply<Ltd.lambda, S>) => {
      switch (true) {
        case Ltd.is.null(x): return x
        case Ltd.is.boolean(x): return x
        case Ltd.is.integer(x): return x
        case Ltd.is.number(x): return x
        case Ltd.is.string(x): return x
        case Ltd.is.array(x): return { ...x, items: f(x.items) }
        case Ltd.is.allOf(x): return { ...x, allOf: x.allOf.map(f) }
        case Ltd.is.anyOf(x): return { ...x, anyOf: x.anyOf.map(f) }
        case Ltd.is.oneOf(x): return { ...x, oneOf: x.oneOf.map(f) }
        case Ltd.is.object(x): 
          return { ...x, properties: Object_fromEntries(Object_entries(x.properties).map(([k, v]) => [k, f(v)])) }
  ...
  export function is(u: unknown): u is Ltd { return true }
  ...
  export namespace is {
    ...
    export const object = <T>(u: unknown): u is Ltd.object.F<T> => isObject(u) && u.type === "object" && !!(u.properties)
  ...
  export namespace make {
    export const boolean   
      : (meta?: Ext.Meta) => Ltd.boolean
      = (meta = {}) => ({ type: "boolean", ...meta })
    export const integer 
      : (meta?: Ext.Meta) => Ltd.integer
      = (meta = {}) => ({ type: "integer", ...meta })
    export const number 
      : (meta?: Ext.Meta) => Ltd.number
      = (meta = {}) => ({ type: "number", ...meta })
    export const string 
      : (meta?: Ext.Meta) => Ltd.string
      = (meta = {}) => ({ type: "string", ...meta })
    export const allOf 
      : <T>(allOf: readonly T[]) => Ext.allOf.F<T>
      = (allOf) => ({ allOf })
    export const anyOf 
      : <T>(anyOf: readonly T[]) => Ext.anyOf.F<T>
      = (anyOf) => ({ anyOf })
    export const oneOf 
      : <T>(oneOf: readonly T[]) => Ext.oneOf.F<T>
      = (oneOf) => ({ oneOf })
    export const array 
      : <T>(items: T, meta?: Ext.Meta) => Ext.array.F<T>
      = (_, meta = {}) => ({ type: "array", items: _, ...meta })
    export const tuple 
      : <T>(items: readonly T[], meta?: Ext.Meta) => Ext.tuple.F<T>
      = (_, meta = {}) => ({ type: "tuple", items: _, ...meta })
    export const record 
      : <T>(additionalProperties: T, meta?: Ext.Meta) => Ext.record.F<T>
      = (_, meta = {}) => ({ type: "record", additionalProperties: _, ...meta })
    export const object 
      : <T>(properties: { [x: string]: T }, meta?: Ext.Meta & { required?: readonly string[] }) => Ext.object.F<T>
  ...
  export namespace Algebra {
    export const count
      : Functor.Algebra<Ltd.lambda, number> 
      = (n) => {
        switch (true) {
          case Ltd.is.null(n): return 1
          case Ltd.is.boolean(n): return 1
          case Ltd.is.integer(n): return 1
          case Ltd.is.number(n): return 1
          case Ltd.is.string(n): return 1
    ...
    export const paths
      : Functor.Algebra<Ltd.lambda, readonly (readonly (keyof any)[])[]> 
      = (n) => {
        switch (true) {
          case Ltd.is.null(n): return [[n.type]]
          case Ltd.is.boolean(n): return [[n.type]]
          case Ltd.is.integer(n): return [[n.type]]
          case Ltd.is.number(n): return [[n.type]]
          case Ltd.is.string(n): return [[n.type]]
...
export declare namespace Ext {
  interface Meta { originalIx?: number }
  interface allOf extends Meta { allOf: readonly Ext[] }
  namespace allOf { interface F<T> extends Meta { allOf: readonly T[] } }
  interface anyOf extends Meta { anyOf: readonly Ext[] }
  namespace anyOf { interface F<T> extends Meta { anyOf: readonly T[] } }
  interface oneOf extends Meta { oneOf: readonly Ext[] }
  namespace oneOf { interface F<T> extends Meta { oneOf: readonly T[] } }
  interface array extends Meta { type: "array", items: Ext }
  namespace array { interface F<T> extends Meta { type: "array", items: T } }
  interface tuple extends Meta { type: "tuple", items: readonly Ext[] }
  namespace tuple { interface F<T> extends Meta { type: "tuple", items: readonly T[] } }
  interface record extends Meta { type: "record", additionalProperties: Ext }
  namespace record { interface F<T> extends Meta { type: "record", additionalProperties: T } }
  interface Ext_object { type: "object", properties: { [x: string]: Ext }, required: readonly string[] }
  namespace Ext_object 
    { interface F<T> extends Meta { type: "object", properties: { [x: string]: T }, required: readonly string[] } }
  ///
  interface lambda<T = unknown> extends HKT<T> { [-1]: Ext.F<this[0]> }
  type F<T> =
    | Ltd.null
    | Ltd.boolean
    | Ltd.integer
...
export namespace Ext {
  ...
  export namespace make {
    ...
    export const array 
      : <T>(items: T, meta?: Ext.Meta) => Ext.array.F<T>
      = (items, meta = {}) => ({ type: "array", items, ...meta })
    export const tuple 
      : <T>(items: readonly T[], meta?: Ext.Meta) => Ext.tuple.F<T>
      = (items, meta = {}) => ({ type: "tuple", items, ...meta })
    export const record 
      : <T>(additionalProperties: T, meta?: Ext.Meta) => Ext.record.F<T>
      = (_, meta = {}) => ({ type: "record", additionalProperties: _, ...meta }) 
    export const object 
  ...
  export function map<S, T>(f: (s: S) => T): (s: Ext.F<S>) => Ext.F<T> {
    return (x: Ext.F<S>) => {
      switch (true) {
        ...
        case Ext.is.tuple(x): return { ...x, items: x.items.map((y, ix) => f(...([y, ix + 10] as never as [S]))) }
  ...
  export function fold<T>(algebra: Functor.Algebra<Ext.lambda, T>) /// impl.
    { return fn.cata(functor)(algebra) }

  export function unfold<T>(coalgebra: Functor.Coalgebra<Ext.lambda, T>): (term: T) => Ext
  export function unfold<T>(coalgebra: Functor.Coalgebra<Ext.lambda, T>) /// impl.
  ...
  export namespace lax {
    ...
    export const isRecordLike = (u: unknown): u is TupleLike => isObject(u) && u.type === "object" && Array_isArray(u.items)
  }

  export namespace Coalgebra {
    export const fromSchema
      : Functor.Coalgebra<Ext.lambda, Ext.lax> 
      = (expr) => {
        const meta = {
          ..."originalIx" in expr && { originalIx: expr.originalIx },
          ..."required" in expr && { required: expr.required },
        } satisfies Ext.Meta
        switch (true) {
          case Ltd.is.null(expr): return Ltd.make.null()
          case Ltd.is.boolean(expr): return Ltd.make.boolean(meta)
          case Ltd.is.integer(expr): return Ltd.make.integer(meta)
          case Ltd.is.number(expr): return Ltd.make.number(meta)
          case Ltd.is.string(expr): return Ltd.make.string(meta)
          case Ltd.is.allOf(expr): return Ext.make.allOf(expr.allOf)
          case Ltd.is.anyOf(expr): return Ext.make.anyOf(expr.anyOf)
          case Ltd.is.oneOf(expr): return Ext.make.oneOf(expr.oneOf)
          case lax.isTupleLike(expr): return Ext.make.tuple(expr.items, meta)
          case lax.isArrayLike(expr): return Ext.make.array(expr.items, meta)
          case lax.isObjectLike(expr): return !("properties" in expr) 
            ? Ext.make.record(expr.additionalProperties, meta)
            : Ext.make.object(expr.properties, { 
              ...meta, 
              required: !core.is.array(core.is.string)(meta.required) ? [] : meta.required 
            })
