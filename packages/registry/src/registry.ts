...
/////////////////////////////
///  (!) SIDE-EFFECT (!)  ///
export function register<const URI extends string, const Symbol extends symbol>(
  uri: URI,
  symbol: Symbol,
...
export function createRegistry<const T extends globalThis.Record<string, symbol>>(
  uriToSymbolMap: T,
...
export function createRegistry<const T extends globalThis.Record<string, symbol>>(uriToSymbolMap: T) {
  for (const uri in uriToSymbolMap) {
    ...
    if (globalThis.process.env) void globalThis.console.info("Registering " + uri + "...")
    void register(uri, sym)
  }
  return {
    get(k: string) {
      return (globalThis.SymbolRegistry as any).get(k as never)
    },
    map: globalThis.SymbolRegistry,
  }
}
