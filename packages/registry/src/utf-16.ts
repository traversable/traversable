import type { HKT, defined, newtype } from "./types.js"
...
declare namespace Template {
  ...
  //////////////
  /// apply
  interface Identity extends HKT<Template.Entry, string> {
    [-1]: defined<this[0]>[0]
  }
  interface Property<_NS extends string = string> extends HKT<Template.Entry, string> {
    [-1]: createProperty<defined<this[0]>>
  }
  interface Emoji<NS extends string> extends HKT<Template.Entry, string> {
    [-1]: createEmoji<defined<this[0]>[0], NS, defined<this[0]>[1]>
  }
  // interface Letter extends HKT<Template.Entry, unknown>
  //   { [-1]: [upper: this[0][0], lower: this[0][1]]  }
  interface Letter<CS extends readonly [upper: string, lower: string]> extends newtype<CS> {}
}

export type JsDoc<F extends HKT<Template.Entry>, T extends { [x: string]: defined<F[0]>[1] }> =
  | never
  | { [K in keyof T]: HKT.apply<F, [T[K] & defined<F[0]>[0], K & defined<F[0]>[1]]> }
