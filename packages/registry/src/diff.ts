...
function diff<const L, const R>(left: L, right: R) { return [left, right, []] }

type Diff<L, R, $ extends Diff.Context = Diff.Context.init>
  = [L, R] extends [Primitive, Primitive] ? Diff.primitives<L, R, $>
  : [L, R] extends [Array, Array] ? Diff.arrays<L, R, $>
  : [L, R] extends [Dict, Dict] ? Diff.objects<L, R, $>
  : Diff.ᐃ<L, R, $["path"]>
  ;

declare namespace Diff {
  type primitives<L, R, _ extends Diff.Context> 
    = L extends R ? never : R extends L ? never : Diff.ᐃ<L, R, _["path"]>
  type objects<
    L, 
    R, 
  ...
  interface Context { path: readonly (keyof any)[] }
  namespace Context {
    type init = never | { path: [] }
    type next<
      Next,
      Prev extends Context = Context, 
    > = never | { path: [...Prev["path"], Next] }
  }
  type LeftOnly<L, R, _ extends readonly unknown[]> = never | { 
    [ K in keyof L as Exclude<keyof L, keyof R>]
    : K extends K ? [ᐩᐮ: L[Exclude<K, keyof R>], ⴰⴰⴰ: [..._, Exclude<K, keyof R>]] : never 
  }
  type RightOnly<L, R, _ extends readonly unknown[]> = never | { 
    [ K in keyof R as Exclude<keyof R, keyof L>]
    : K extends K ? [ᐩᐭ: R[Exclude<K, keyof L>], ⴰⴰⴰ: [..._, Exclude<K, keyof L>]] : never 
  }
