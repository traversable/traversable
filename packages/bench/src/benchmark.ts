...
type OptionalKeys<T, K extends keyof T = keyof T> = K extends K
	? {} extends globalThis.Pick<T, K>
		...
		: never
	: never;
type OptionalPart<T, K extends OptionalKeys<T> = OptionalKeys<T>> =
	| never
	| { [P in K]: T[P] };
...
export declare namespace bench {
	...
	interface Options
		extends SharedOptions,
			globalThis.Omit<tinybench.Options, "name"> {}

	interface Config
		extends globalThis.Required<SharedOptions>,
			globalThis.Omit<tinybench.Options, "name"> {}
...
export namespace bench {
	...
	export const defaultTimestamp = (() =>
		new globalThis.Date()
			.toISOString()
			.substring(0, 19)) satisfies bench.Config["timestamp"];
	export const defaultGenerateHash = (() =>
		fc
			.sample(fc.uuid(), 1)[0]
			.substring(0, 8)) satisfies bench.Config["generateHash"];
	export const defaultReporter =
		defaultStringify satisfies bench.Config["reporter"];
	...
	export function configFromOptions<T>({
		outdir,
		benchmarks,
		...opt
	}: bench.Options): Config {
	...
	export const rootReporter: (
		deps: bench.RootReporterDeps,
