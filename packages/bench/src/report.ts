...
export const buildTable = (path: string): Table => {
	...
	return {
		...
		SUMM: (duration: bigint) =>
			`│ ⌛️ │ Benchmark duration │ ${getSummary(duration)} │`,
		FOOT: `└────┴────────────────────┴─${"─".repeat(pad)}─┘`,
...
const cleanup: (
	table: Table,
	duration: readonly [start: bigint, end: bigint],
) => void = (table, [start, end]) => {
	void log(table.DONE);
...
export const run: (options: Options) => () => void = (options) => () => {
	...
	const benchmarks = fs
		...
		.filter(
			(dirent) => dirent.isFile() && !config.exclude.includes(dirent.name),
		)
