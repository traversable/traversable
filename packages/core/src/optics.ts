...
export namespace Either {
  ...
  export function map<A, B>(f: (a: A) => B): Either.map<A, B> 
    { return (either) => Either.isRight(either) ? Either.right(f(either.right)) : either }
  export function mapLeft<S, T>(f: (s: S) => T): Either.mapLeft<S, T>
    { return (either) => Either.isLeft(either) ? Either.left(f(either.left)) : either }
  export function mapBoth<S, T, A, B>(f: (a: A) => B, g: (s: S) => T): Either.mapBoth<S, T, A, B>
...
export namespace Strong {
  ...
  export function first<A, B>(f: (a: A) => B): Strong.first<A, B>
    { return ([a, t]) => Tuple.of(f(a), t) }
  export function second<A, B>(f: (a: A) => B): Strong.second<A, B>
...
export namespace Choice {
  ...
  export function left<S, T>(f: (s: S) => T): Choice.left<S, T> 
    { return (either) => Either.isLeft(either) ? Either.left(f(either.left)) : either }
  export function right<A, B>(f: (a: A) => B): Choice.right<A, B> 
    { return (either) => Either.isRight(either) ? Either.right(f(either.right)) : either }
}
