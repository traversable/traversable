...
vi.describe("〖⛳️〗‹‹‹ ❲@traversable/core/ast❳", () => {
  ...
  vi.it("〖⛳️〗› ❲ast: `ast.toJsonSchema` adapter❳", () => {
    ...
    vi.expect($(seed.const_01.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"const","enum":[null]}"`)
    vi.expect($(seed.const_02.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"const","enum":[false]}"`)
    vi.expect($(seed.const_03.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"const","enum":[0]}"`)
    vi.expect($(seed.const_04.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"const","enum":[""]}"`)
    vi.expect($(seed.const_06.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"const","enum":[{}]}"`)
    ...
    vi.expect($(seed.optional_07.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"const","enum":[null]}"`)
    ...
    vi.expect($(seed.tuple_00.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[]}"`)
    vi.expect($(seed.tuple_01.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"null","enum":[null]}]}"`)
    vi.expect($(seed.tuple_02.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"boolean"}]}"`)
    vi.expect($(seed.tuple_03.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"integer"}]}"`)
    vi.expect($(seed.tuple_04.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"number"}]}"`)
    vi.expect($(seed.tuple_05.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"string"}]}"`)
    vi.expect($(seed.tuple_06.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"null","enum":[null]},{"type":"null","enum":[null]}]}"`)
    vi.expect($(seed.tuple_07.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"boolean"},{"type":"boolean"}]}"`)
    vi.expect($(seed.tuple_08.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"integer"},{"type":"integer"}]}"`)
    vi.expect($(seed.tuple_09.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"number"},{"type":"number"}]}"`)
    vi.expect($(seed.tuple_10.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"string"},{"type":"string"}]}"`)
    vi.expect($(seed.tuple_11.toJsonSchema)).toMatchInlineSnapshot(`"{"type":"array","items":[{"type":"string"},{"type":"string"}]}"`)
    ...
    vi.expect($(seed.allOf_04.toJsonSchema)).toMatchInlineSnapshot(`"{"allOf":[{"type":"object","required":[],"properties":{"a":{"type":"const","enum":[100]}}},{"type":"object","required":["b"],"properties":{"b":{"type":"array","items":{"type":"string"}}}}]}"`)
    ...
    vi.expect($(seed.anyOf_04.toJsonSchema)).toMatchInlineSnapshot(`"{"anyOf":[{"type":"object","required":[],"properties":{"a":{"type":"const","enum":[100]}}},{"type":"object","required":["b"],"properties":{"b":{"type":"array","items":{"type":"string"}}}}]}"`)
    ...
    vi.assertType<{ enum: [null]; type: "const"; }>(seed.const_01.toJsonSchema)
    vi.assertType<{ enum: [false]; type: "const"; }>(seed.const_02.toJsonSchema)
    vi.assertType<{ enum: [0]; type: "const"; }>(seed.const_03.toJsonSchema)
    vi.assertType<{ enum: [""]; type: "const"; }>(seed.const_04.toJsonSchema)
    vi.assertType<{ enum: [readonly []]; type: "const"; }>(seed.const_05.toJsonSchema)
    vi.assertType<{ enum: [readonly []]; type: "const"; }>(seed.const_05.toJsonSchema)
    vi.assertType<{ enum: [{}], type: "const" }>(seed.const_06.toJsonSchema)
    ...
    vi.assertType<{ enum: [null], type: "const" }>(seed.optional_07.toJsonSchema)
    ...
    vi.assertType<{ allOf: [ { type: "object", required: [], properties: { a: { enum: [100], type: "const" } } }, { type: "object", required: "b"[], properties: { b: { type: "array", items: { type: "string" } } } } ] }>(seed.allOf_04.toJsonSchema)
    ...
    vi.assertType<{ anyOf: [ { type: "object", required: [], properties: { a: { enum: [100], type: "const" } } }, { type: "object", required: "b"[], properties: { b: { type: "array", items: { type: "string" } } } } ], }>(seed.anyOf_04.toJsonSchema)
  })
})
